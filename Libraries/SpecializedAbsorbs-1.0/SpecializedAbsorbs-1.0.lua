...
if oldminor then
	...
	-- No AceComm/AceSerializer: stub them out, cross-player stat sync disabled
	function Core:RegisterComm() end
	function Core:UnregisterAllComm() end
	function Core:SendCommMessage() end
	function Core:Serialize(...) return "" end
	function Core:Deserialize() return false end

	-- Pure frame-based timer replacement for AceTimer-3.0
	do
		...
		function Core:ScheduleTimer(func, delay, arg)
			uid = uid + 1
			timers[uid] = {func = func, remaining = delay, arg = arg}
			return uid
		end

		function Core:ScheduleRepeatingTimer(func, interval, arg)
			uid = uid + 1
			timers[uid] = {func = func, remaining = interval, interval = interval, arg = arg}
			repeating[uid] = true
			return uid
		end

		function Core:CancelTimer(id)
			if id then
				timers[id] = nil
				repeating[id] = nil
			end
	...
	function Core:ScheduleUniqueTimer(id, callback, delay, arg)
		if not activeTimers[id] then
			local handle = self:ScheduleTimer(function(a)
				callback(a)
				activeTimers[id] = nil
			end, delay, arg)
			activeTimers[id] = handle
		end
	end
end
...
do
	...
	function SortEffects(a, b)
		if a[1] == b[1] then
			if a[8] == nil then return true
			elseif b[8] == nil then return false
			else return (a[8] < b[8]) end
		end
		if a[1] == 65686 then return true end
		if b[1] == 65686 then return false end
		if a[1] == 65684 then return true end
		if b[1] == 65684 then return false end
		if tContains(mage_frost_ward, a[1]) then return true end
		if tContains(mage_frost_ward, b[1]) then return false end
		if tContains(mage_fire_ward, a[1]) then return true end
		if tContains(mage_fire_ward, b[1]) then return false end
		if tContains(warlock_shadow_ward, a[1]) then return true end
		if tContains(warlock_shadow_ward, b[1]) then return false end
		if a[1] == 58597 then return true end
		if b[1] == 58597 then return false end
		if a[1] == 47753 then return true end
		if b[1] == 47753 then return false end
		if tContains(mage_ice_barrier, a[1]) then return true end
		if tContains(mage_ice_barrier, b[1]) then return false end
		if tContains(warlock_sacrifice, a[1]) then return true end
		if tContains(warlock_sacrifice, b[1]) then return false end
		return (a[7] < b[7])
	end
end

local function GetUnitId(guid, name)
	if name and UnitGUID(name) then return name end
	...
	if prefix then
		for i = 1, groupCount do
			...
			if UnitExists(unit) then
				...
				if UnitExists(unit .. "pet") and UnitGUID(unit .. "pet") == guid then return unit .. "pet" end
			end
		end
	end

	if UnitGUID("player") == guid then return "player" end
	if UnitExists("playerpet") and UnitGUID("playerpet") == guid then return "playerpet" end
end
...
local function UnitDispatch(func, guid, name)
	...
	if not result then
		...
		if unit then result = func(unit) end
	end
	return result
end
...
function Core.Enable()
	...
	if Events.PLAYER_LEVEL_UP then Core.RegisterEvent("PLAYER_LEVEL_UP") end
	if Events.PLAYER_TALENT_UPDATE then Core.RegisterEvent("PLAYER_TALENT_UPDATE") end
	if Events.GLYPH_UPDATED then Core.RegisterEvent("GLYPH_UPDATED") end
	if Events.PLAYER_EQUIPMENT_CHANGED then Core.RegisterEvent("PLAYER_EQUIPMENT_CHANGED") end
...
function Core.ApplySingularEffect(timestamp, srcGUID, srcName, dstGUID, dstName, spellid, spellschool)
	...
	elseif not destEffects[spellid] then
		...
		Core:CancelTimer(effectEntry[6])
	end
...
function Core.PopCharge(guid, spellid)
	...
	if guidCharges then
		...
		if queue and queue[2] then
			local chargeAmount, chargeExpire
			while true do
...
function Events.STATS_CHANGED()
	...
	-- comm disabled, nothing to broadcast
end
...
function lib.UnitScaling(guid, defaultScaling, defaultQuality)
	...
	if guidScaling then return guidScaling, 1.0 end
...
function lib.UnitStatsAndScaling(guid, missingQuality, defaultScaling, defaultQuality)
	...
	local guidScaling = Scaling and Scaling[guid]
	if guidStats then
		if guidScaling then return guidStats[2], guidStats[3], guidStats[4], guidScaling, 1.0 end
		return guidStats[2], guidStats[3], guidStats[4], defaultScaling, defaultQuality
	else
		if guidScaling then return 0, 0, missingQuality, guidScaling, 1.0 end
...
function lib.UnitTotalHealAbsorbs(guid)
	if GUIDtoAbsorbHealSpells then
		return guid and GUIDtoAbsorbHealSpells[guid]
	end
end
...
local function Physical_Hit(effectEntry, absorbedRemaining, overkill, spellschool)
	if spellschool ~= SCHOOL_MASK_PHYSICAL then return 0, true end
...
local function Holy_Hit(effectEntry, absorbedRemaining, overkill, spellschool)
	if spellschool ~= SCHOOL_MASK_HOLY then return 0, true end
	return generic_Hit(effectEntry, absorbedRemaining, overkill, spellschool)
end
...
local function Nature_Hit(effectEntry, absorbedRemaining, overkill, spellschool)
	if spellschool ~= SCHOOL_MASK_NATURE then return 0, true end
	return generic_Hit(effectEntry, absorbedRemaining, overkill, spellschool)
end
...
local function FrostFire_Hit(effectEntry, absorbedRemaining, overkill, spellschool)
	if spellschool ~= (SCHOOL_MASK_FIRE + SCHOOL_MASK_FROST) then return 0, true end
	return generic_Hit(effectEntry, absorbedRemaining, overkill, spellschool)
end
...
local function Arcane_Hit(effectEntry, absorbedRemaining, overkill, spellschool)
	if spellschool ~= SCHOOL_MASK_ARCANE then return 0, true end
	return generic_Hit(effectEntry, absorbedRemaining, overkill, spellschool)
end
...
local function deathknight_AntiMagicShell_Create(srcGUID, srcName, dstGUID, dstName, spellid, destEffects)
	...
	if maxHealth == 0 then return 0, 0.0 end
...
local function deathknight_AntiMagicShell_Hit(effectEntry, absorbedRemaining, overkill, spellschool)
	...
	if effectEntry[3] < maxAbsorb then return effectEntry[3], false end
...
local mage_Absorb_Spells = {
	[543] = 165, [8457] = 290, [8458] = 470, [10223] = 675, [10225] = 875, [27218] = 1125, [43010] = 1950,
	[6143] = 165, [8461] = 290, [8462] = 470, [10177] = 675, [28609] = 875, [32796] = 1125, [43012] = 1950,
	[11426] = 438, [13031] = 549, [13032] = 678, [13033] = 818, [27134] = 925, [33405] = 1075, [43038] = 2800, [43039] = 3300,
	[1463] = 120, [8494] = 210, [8495] = 300, [10191] = 390, [10192] = 480, [10193] = 570, [27131] = 715, [43019] = 1080, [43020] = 1330
}
...
local priest_PWS_Ranks = {
	[1] = {17, 6, 44, 4}, [2] = {592, 12, 88, 6}, [3] = {600, 18, 158, 8},
	[4] = {3747, 24, 234, 10}, [5] = {6065, 30, 301, 11}, [6] = {6066, 36, 381, 13},
	[7] = {10898, 42, 484, 15}, [8] = {10899, 48, 605, 17}, [9] = {10900, 54, 763, 19},
	[10] = {10901, 60, 942, 21}, [11] = {25217, 65, 1125, 18}, [12] = {25218, 70, 1265, 20},
	[13] = {48065, 75, 1920, 30}, [14] = {48066, 80, 2230, 0}
}
...
local function priest_DivineAegis_Create(srcGUID, srcName, dstGUID, dstName, spellid, destEffects)
	...
	local quality = 1.0
	if destLevel == 0 then destLevel = 80; quality = 0.4 end
...
local function priest_ScanTalents()
	privateScaling["TwinDisc"] = select(5, GetTalentInfo(1, 2))
	privateScaling["ImpPWS"] = select(5, GetTalentInfo(1, 9))
	privateScaling["FocusedPower"] = select(5, GetTalentInfo(1, 16))
	privateScaling["DivineAegis"] = select(5, GetTalentInfo(1, 24))
	privateScaling["BorrowedTime"] = select(5, GetTalentInfo(1, 27))
	privateScaling["SpiritualHealing"] = select(5, GetTalentInfo(2, 16))
end
...
local function priest_ScanEquipment()
	...
	privateScaling["4pcRaid9"] = (n >= 4) and 1 or 0
end
...
function OnEnableClass.PRIEST()
	Events.PLAYER_LEVEL_UP = function() priest_UpdatePlayerScaling() end
	Events.PLAYER_TALENT_UPDATE = function() priest_ScanTalents(); priest_UpdatePlayerScaling() end
...
local warlock_Sacrifice_Spells = {
	[7812] = 305, [19438] = 510, [19440] = 770, [19441] = 1095,
	[19442] = 1470, [19443] = 1905, [27273] = 2855, [47985] = 6750, [47986] = 8365
}

local warlock_ShadowWard_Spells = {
	[6229] = 290, [11739] = 470, [11740] = 675, [28610] = 875, [47890] = 2750, [47891] = 3300
}
...
local function items_EssenceOfGossamer_Hit(effectEntry)
	if effectEntry[3] < 140 then return effectEntry[3], false end
	return 140, true
end
...
Core.Effects = {
	...
	[543] = mage_FireWard_Entry, [8457] = mage_FireWard_Entry, [8458] = mage_FireWard_Entry,
	[10223] = mage_FireWard_Entry, [10225] = mage_FireWard_Entry, [27218] = mage_FireWard_Entry, [43010] = mage_FireWard_Entry,
	[6143] = mage_FrostWard_Entry, [8461] = mage_FrostWard_Entry, [8462] = mage_FrostWard_Entry,
	[10177] = mage_FrostWard_Entry, [28609] = mage_FrostWard_Entry, [32796] = mage_FrostWard_Entry, [43012] = mage_FrostWard_Entry,
	[11426] = mage_IceBarrier_Entry, [13031] = mage_IceBarrier_Entry, [13032] = mage_IceBarrier_Entry, [13033] = mage_IceBarrier_Entry,
	[27134] = mage_IceBarrier_Entry, [33405] = mage_IceBarrier_Entry, [43038] = mage_IceBarrier_Entry, [43039] = mage_IceBarrier_Entry,
	[1463] = mage_ManaShield_Entry, [8494] = mage_ManaShield_Entry, [8495] = mage_ManaShield_Entry,
	[10191] = mage_ManaShield_Entry, [10192] = mage_ManaShield_Entry, [10193] = mage_ManaShield_Entry,
	[27131] = mage_ManaShield_Entry, [43019] = mage_ManaShield_Entry, [43020] = mage_ManaShield_Entry,
	[58597] = {1.0, 6, paladin_SacredShield_Create, generic_Hit},
	[17] = priest_PWS_Entry, [592] = priest_PWS_Entry, [600] = priest_PWS_Entry,
	[3747] = priest_PWS_Entry, [6065] = priest_PWS_Entry, [6066] = priest_PWS_Entry,
	[10898] = priest_PWS_Entry, [10899] = priest_PWS_Entry, [10900] = priest_PWS_Entry,
	[10901] = priest_PWS_Entry, [25217] = priest_PWS_Entry, [25218] = priest_PWS_Entry,
	[48065] = priest_PWS_Entry, [48066] = priest_PWS_Entry,
	[47753] = {1.0, 12, priest_DivineAegis_Create, generic_Hit},
	[52179] = {2.5, nil, shaman_AstralShift_Create, shaman_AstralShift_Hit},
	[7812] = warlock_Sacrifice_Entry, [19438] = warlock_Sacrifice_Entry, [19440] = warlock_Sacrifice_Entry,
	[19441] = warlock_Sacrifice_Entry, [19442] = warlock_Sacrifice_Entry, [19443] = warlock_Sacrifice_Entry,
	[27273] = warlock_Sacrifice_Entry, [47985] = warlock_Sacrifice_Entry, [47986] = warlock_Sacrifice_Entry,
	[6229] = warlock_ShadowWard_Entry, [11739] = warlock_ShadowWard_Entry, [11740] = warlock_ShadowWard_Entry,
	[28610] = warlock_ShadowWard_Entry, [47890] = warlock_ShadowWard_Entry, [47891] = warlock_ShadowWard_Entry,
...
Core.CombatTriggers = {
	OnAuraApplied = {[64411] = items_Valanyr_OnAuraApplied},
	OnAuraRemoved = {[64411] = items_Valanyr_OnAuraRemoved},
	OnHeal = {},
	OnHealCrit = {}
}

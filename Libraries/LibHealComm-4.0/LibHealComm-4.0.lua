...
local function parseHealDelayed(casterGUID, startTimeRelative, endTimeRelative, spellID)
	...
	-- Fix: Check if the parent tables exist before indexing
	local pending = (pendingHeals[casterGUID] and pendingHeals[casterGUID][spellName]) 
	             or (pendingHots[casterGUID] and pendingHots[casterGUID][spellName])
	
	-- Add safety check in case pending is nil

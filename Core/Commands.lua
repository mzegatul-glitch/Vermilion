...
-- Fix The CombatLog.
SlashCmdList.CLEARCOMBAT = function() CombatLogClearEntries() V.Print("|cffffe02eCombatLog has been cleared & fixed.|r") end
...
SlashCmdList["GROUPDISBAND"] = function()
	...
	if UnitInRaid("player") then
		for i = 1, GetNumRaidMembers() do
			...
			if online and name ~= V.Name then
				UninviteUnit(name)
			end
		end
	else
...
function SlashCmdList.LUAERROR(msg)
	...
	if(msg == "on") then
		DisableAllAddOns()
		EnableAddOn("Vermilion")
		EnableAddOn("Vermilion_Config")
		SetCVar("scriptErrors", 1)
		ReloadUI()
	elseif(msg == "off") then
		SetCVar("scriptErrors", 0)
		V.Print("Lua errors off.")
	else
		V.Print("/luaerror on - /luaerror off")
	end
end
...
SlashCmdList.PARTYTORAID = function()
	if GetNumPartyMembers() > 0 then
		...
		V.Print("|cffffe02e"..ERR_NOT_IN_GROUP.."|r")
	end
end
...
-- Grid on screen
V.GridFrame = nil
V.GridFrame = CreateFrame("Frame", nil, UIParent)
SlashCmdList.GRIDONSCREEN = function()
	if grid then
		...
		local width = UIParent:GetWidth() / 128
		local height = UIParent:GetHeight() / 72
		local centerX = 64
		local centerY = 36
		for i = 0, 128 do
			...
			texture:SetPoint("BOTTOMRIGHT", grid, "TOPLEFT", i * width, -UIParent:GetHeight())
		end
		for i = 0, 72 do
			local texture = grid:CreateTexture(nil, "BACKGROUND")
			if i == centerY then
		...
		-- Center dot
		local center = grid:CreateTexture(nil, "OVERLAY")
		center:SetTexture(46/255, 181/255, 255/255, 1)
		center:SetSize(6, 6)

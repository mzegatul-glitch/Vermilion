...
rightbaranchor:SetWidth((C.ActionBar.ButtonSize * 8) + (C.ActionBar.ButtonSpace * 6))

if C.ActionBar.RightBars == 1 then
	rightbaranchor:SetHeight((C.ActionBar.ButtonSize * 2) + C.ActionBar.ButtonSpace)

elseif C.ActionBar.RightBars == 2 then
	rightbaranchor:SetHeight((C.ActionBar.ButtonSize * 2) + C.ActionBar.ButtonSpace)

elseif C.ActionBar.RightBars == 3 then
	rightbaranchor:SetHeight((C.ActionBar.ButtonSize * 2) + (C.ActionBar.ButtonSpace * 2))
...
if C.ActionBar.PetBarHorizontal == true then
	petbaranchor:CreatePanel("Invisible", (C.ActionBar.ButtonSize * 7.8) + (C.ActionBar.ButtonSpace * 9), (C.ActionBar.ButtonSize + C.ActionBar.ButtonSpace), unpack(C.Position.PetHorizontal))
elseif C.ActionBar.RightBars > 0 then
	petbaranchor:CreatePanel("Invisible", C.ActionBar.ButtonSize + 3, (C.ActionBar.ButtonSize * 7.8) + (C.ActionBar.ButtonSpace * 9), "RIGHT", rightbaranchor, "LEFT", 0, 0)
else
	petbaranchor:CreatePanel("Invisible", (C.ActionBar.ButtonSize + C.ActionBar.ButtonSpace), (C.ActionBar.ButtonSize * 7.8) + (C.ActionBar.ButtonSpace * 9), unpack(C.Position.RightBars))
end

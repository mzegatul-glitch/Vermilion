...
if (select(4, GetAddOnInfo("SexyMap"))) or (select(4, GetAddOnInfo("wMinimap"))) then
	C.Minimap.Enable = true
end

if (select(4, GetAddOnInfo("Stuf"))) or (select(4, GetAddOnInfo("PitBull4"))) or (select(4, GetAddOnInfo("ShadowedUnitFrames"))) or (select(4, GetAddOnInfo("RUF"))) then
...
if (select(4, GetAddOnInfo("Vermilion_OldBars"))) then
	C.ActionBar.Enable = false
end

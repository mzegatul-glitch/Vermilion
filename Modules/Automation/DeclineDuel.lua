...
DeclineDuel:SetScript("OnEvent", function(self, event, name)
	...
	if event == "DUEL_REQUESTED" then
		...
		V.Print(format("|cff2eb6ff"..L_INFO_DUEL.."|cffffe02e"..name.."."))
...
SlashCmdList.DISABLEDECLINE = function()
	if not Disable then
		Disable = true
		V.Print("|cffffe02eDueling is now|r |cFF008000enabled|r")
	else
		Disable = false
		V.Print("|cffffe02eDueling is now|r |cFFFF0000disabled|r")
	end
end

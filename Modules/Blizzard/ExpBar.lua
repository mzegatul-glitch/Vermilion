...
backdrop:SetPoint("TOPLEFT", Minimap, "BOTTOMLEFT", -1, -27)
backdrop:SetPoint("TOPRIGHT", Minimap, "BOTTOMRIGHT", 1, -27)
backdrop:SetBackdrop(V.BorderBackdrop)
...
mouseFrame:SetScript("OnEnter", function()
	...
	if UnitLevel("player") ~= MAX_PLAYER_LEVEL then
		GameTooltip:AddDoubleLine(L_EXPERIENCE_BAR, V.Name, r, g, b, 1, 1, 1)
		GameTooltip:AddDoubleLine(L_CURRENT_EXPERIENCE, format('%s/%s (%d%%)', V.Comma(XP), V.Comma(maxXP), (XP/maxXP)*100), r, g, b, 1, 1, 1)
		GameTooltip:AddDoubleLine(L_REMAINING_EXPERIENCE, format('%s', V.Comma(maxXP-XP)), r, g, b, 1, 1, 1)
		if restXP then
			GameTooltip:AddDoubleLine(L_RESTED_EXPERIENCE, format('|cffb3e1ff%s (%d%%)', V.Comma(restXP), restXP/maxXP*100), r, g, b)
		end
	end
	if GetWatchedFactionInfo() then
		...
		GameTooltip:AddDoubleLine(L_CURRENT_REPUTATION, format('%s/%s (%d%%)', V.Comma(value-start), V.Comma(cap-start), (value-start)/(cap-start)*100), r, g, b, 1, 1, 1)
		GameTooltip:AddDoubleLine(L_REMAINING_REPUTATION, format('%s', V.Comma(cap-value)), r, g, b, 1, 1, 1)
	end
	GameTooltip:Show()
end)

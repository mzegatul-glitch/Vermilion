...
local function CreateRollButton(parent, ntex, ptex, htex, rolltype, tiptext, ...)
	...
	txt:SetShadowOffset((V.Mult or 1), -(V.Mult or 1))
...
local function CreateRollFrame()
	...
	V.CreateBorder(frame, 10, 2)
	frame:SetBackdrop(V.BorderBackdrop)
	...
	V.CreateBorder(button, 10, 2)
	button:SetBackdrop(V.BorderBackdrop)
	...
	bind:SetShadowOffset((V.Mult or 1), -(V.Mult or 1))
	frame.fsbind = bind

	local loot = frame:CreateFontString(nil, "ARTWORK")
	loot:SetFont(C.Media.Font, C.Media.Font_Size, C.Media.Font_Style)
	loot:SetShadowOffset((V.Mult or 1), -(V.Mult or 1))
...
local function START_LOOT_ROLL(rollid, time)
	...
	if C.Loot.AutoGreed and V.Level == MAX_PLAYER_LEVEL and quality == 2 and not bop then return end

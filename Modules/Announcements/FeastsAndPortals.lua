...
frame:SetScript("OnEvent", function(self, _, ...)
	...
	if subEvent == "SPELL_CAST_SUCCESS" then
		...
		if C.Announcements.Feasts and (spellID == 46887 or spellID == 43015 or spellID == 43478 or spellID == 34753 or spellID == 45279 or spellID == 43480) then
			SendChatMessage(format(L_ANNOUNCE_FP_PRE, srcName, GetSpellLink(spellID)), V.CheckChat(true))
		-- Refreshment Table
		...
		elseif C.Announcements.Portals and (spellID == 698 or spellID == 29893 or spellID == 58887) then
			SendChatMessage(format(L_ANNOUNCE_FP_CLICK, srcName, GetSpellLink(spellID)), V.CheckChat(true))
		-- Piccolo of the Flaming Fire
		elseif C.Announcements.Toys and (spellID == 18400) then
			SendChatMessage(format(L_ANNOUNCE_FP_USE, srcName, GetSpellLink(spellID)), V.CheckChat(true))
		end
	elseif subEvent == "SPELL_SUMMON" then
		-- Repair Bots
		if C.Announcements.Feasts and (V.AnnounceBots[spellID]) then
			SendChatMessage(format(L_ANNOUNCE_FP_PUT, srcName, GetSpellLink(spellID)), V.CheckChat(true))
		end
	elseif subEvent == "SPELL_CREATE" then
		...
		if C.Announcements.Feasts and (spellID == 54710) then
			SendChatMessage(format(L_ANNOUNCE_FP_PUT, srcName, GetSpellLink(spellID)), V.CheckChat(true))
		-- Toys
		elseif C.Announcements.Toys and (V.AnnounceToys[spellID]) then
			SendChatMessage(format(L_ANNOUNCE_FP_PUT, srcName, GetSpellLink(spellID)), V.CheckChat(true))
		-- Portals
		elseif C.Announcements.Portals and (V.AnnouncePortals[spellID]) then
			SendChatMessage(format(L_ANNOUNCE_FP_CAST, srcName, GetSpellLink(spellID)), V.CheckChat(true))
		end
	elseif subEvent == "SPELL_AURA_APPLIED" then
		-- Turkey Feathers and Party G.R.E.N.A.D.E.
		if C.Announcements.Toys and (spellID == 61781 or ((spellID == 51508 or spellID == 51510) and destName == V.Name)) then
			SendChatMessage(format(L_ANNOUNCE_FP_USE, srcName, GetSpellLink(spellID)), V.CheckChat(true))
		end
	end
end)
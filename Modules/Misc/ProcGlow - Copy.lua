...
ProcGlow.SetupArrow = function() end

local function BuildConfigUI()
    if configFrame then configFrame:Show(); return end

    local f = CreateFrame("Frame", "ProcGlowConfig", UIParent, "BackdropTemplate")
    f:SetSize(480, 620)
    ...
    for _, entry in ipairs(uniqueProcs) do
        ...
        texCB:SetChecked(not (DB.disabledTextures and DB.disabledTextures[pID]))
        texCB:SetEnabled(not (DB.disabledProcs and DB.disabledProcs[pID]))

...
frame:SetBackdrop({
	...
	insets = {top = 3 * V.NoScaleMult, left = 3 * V.NoScaleMult, bottom = 3 * V.NoScaleMult, right = 3 * V.NoScaleMult}
})
...
icon:SetPoint("TOPLEFT", frame, "TOPLEFT", V.NoScaleMult * 2, -V.NoScaleMult * 2)
icon:SetPoint("BOTTOMRIGHT", frame, "BOTTOMRIGHT", -V.NoScaleMult * 2, V.NoScaleMult * 2)
	...
	if elapsed > 0.05 then
		for i, v in pairs(watching) do
			if GetTime() >= v[1] + 0.5 + threshold then
				if V.pulse_ignored_spells[i] then
					watching[i] = nil
				else
...
function frame:ADDON_LOADED(addon)
	for _, v in pairs(V.pulse_ignored_spells) do
		V.pulse_ignored_spells[v] = true
	end

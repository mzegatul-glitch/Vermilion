		...
		if not bar then
			...
			if self.Mode == "ICON" then
				...
				if bar.statusbar then
					...
					bar.statusbar:SetStatusBarColor(V.Color.r, V.Color.g, V.Color.b, 1)
				...
				if bar.background then
					...
					bar.background:SetVertexColor(V.Color.r, V.Color.g, V.Color.b, 0.2)
				end
...
function Filger:OnEvent(event, unit)
	if event == "SPELL_UPDATE_COOLDOWN" or event == "PLAYER_TARGET_CHANGED" or event == "PLAYER_FOCUS_CHANGED" or event == "PLAYER_ENTERING_WORLD" or event == "UNIT_AURA" and (unit == "target" or unit == "player" or unit == "pet" or unit == "focus") then
		...
		for i = 1, #C["filger_spells"][V.Class][id], 1 do
			local data = C["filger_spells"][V.Class][id][i]
			local found = false
			...
			if found then
				...
				if not self.actives[i] then
					...
					needUpdate = true
					if V.Class == "DEATHKNIGHT" and self.actives[i].duration == 10 then
						self.actives[i] = nil
					end
				else
...
if C["filger_spells"] and C["filger_spells"]["ALL"] then
	if not C["filger_spells"][V.Class] then
		C["filger_spells"][V.Class] = {}
	end
	...
	for i = 1, #C["filger_spells"]["ALL"], 1 do
		...
		for j = 1, #C["filger_spells"][V.Class], 1 do
			spellListClass = C["filger_spells"][V.Class][j]
		...
		if not merge or not spellListClass then
			table.insert(C["filger_spells"][V.Class], C["filger_spells"]["ALL"][i])
		else
...
if C["filger_spells"] and C["filger_spells"][V.Class] then
	for index in pairs(C["filger_spells"]) do
		if index ~= V.Class then
			C["filger_spells"][index] = nil
		end
	end

	local idx = {}
	for i = 1, #C["filger_spells"][V.Class], 1 do
		local jdx = {}
		local data = C["filger_spells"][V.Class][i]
		...
		for j = 1, #data, 1 do
			...
			if not spn and not data[j].slotID then
				V.Print("|cffff0000WARNING: spell/slot ID ["..(data[j].spellID or data[j].slotID or "UNKNOWN").."] no longer exists! Report this to Vermilion.|r")
				table.insert(jdx, j)
			end
		end
		...
		if #data == 0 then
			V.Print("|cffff0000WARNING: section ["..data.Name.."] is empty! Report this to Vermilion.|r")
			table.insert(idx, i)
		end
	end

	for _, v in ipairs(idx) do
		table.remove(C["filger_spells"][V.Class], v)
	end
	...
	for i = 1, #C["filger_spells"][V.Class], 1 do
		...
		if C.Filger.TestMode then
			frame.actives = {}
			for j = 1, math.min(C.Filger.MaxTestIcon, #C["filger_spells"][V.Class][i]), 1 do
				local data = C["filger_spells"][V.Class][i][j]
				local name, icon
				if data.spellID then
					name, _, icon = GetSpellInfo(data.spellID)
				elseif data.slotID then
			...
			for j = 1, #C["filger_spells"][V.Class][i], 1 do
				local data = C["filger_spells"][V.Class][i][j]

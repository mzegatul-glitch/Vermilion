...
local function SetChatStyle(frame)
	...
	if C.Chat.TabsMouseover ~= true then
		...
		EditBoxBackground:SetBackdrop(V.Backdrop)
...
local function SetupChat(self)
	for i = 1, NUM_CHAT_WINDOWS do
		...
		chat:SetMaxResize(2000, 1200)
		chat:SetMinResize(200, 100)
...
local function SetupChatPosAndFont(self)
	for i = 1, NUM_CHAT_WINDOWS do
		...
		if C.Chat.Outline == true then
			...
			chat:SetShadowOffset((V.Mult or 1), -(V.Mult or 1))
		end
		...
		elseif i == 2 then
			if C.Chat.CombatLog ~= true then
				...
				ChatFrame2Tab.SetText = V.Noop
				ChatFrame2Tab:SetWidth(0.001)
				ChatFrame2Tab.SetWidth = V.Noop
			end
		end
	end
...
function SlashCmdList.BIGCHAT(msg, editbox)
	if bigchat == false then
		...
		bigchat = true
		V.Print(L_CHAT_BIGCHAT_ON)
	else
		ChatFrame1:SetSize(360, 210)
		bigchat = false
		V.Print(L_CHAT_BIGCHAT_OFF)
	end
end

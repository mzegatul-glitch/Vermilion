...
local function Timer_OnUpdate(self, elapsed)
	...
	if remain > 0.05 then
		if (self.fontScale * self:GetEffectiveScale() / UIParent:GetScale()) < MIN_SCALE then
			...
			timervalue, formatid, self.nextUpdate = V.GetTimeInfo(remain, C.Cooldown.Threshold)
			self.text:SetFormattedText(("%s%s|r"):format(V.TimeColors[formatid], V.TimeFormats[formatid][2]), timervalue)
		end
	else
		Timer_Stop(self)
	end
